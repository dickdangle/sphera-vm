...
def build_program() -> bytes:
    ...
    asm.instr("b", "skip_phase_forward", cond="NE")
    asm.instr("add_imm", 3, 3, 32)
    asm.label("skip_phase_forward")
    asm.instr("tst_imm", 2, KEY_B)
    asm.instr("b", "skip_phase_back", cond="NE")
    asm.instr("cmp_imm", 3, 32)
    asm.instr("sub_imm", 3, 3, 32, cond="CS")
    asm.label("skip_phase_back")
    asm.instr("tst_imm", 2, KEY_RIGHT)
    asm.instr("b", "skip_right", cond="NE")
    asm.instr("cmp_imm", 4, MAX_X)
    asm.instr("add_imm", 4, 4, MOVE_STEP, cond="LT")
    asm.label("skip_right")
    asm.instr("tst_imm", 2, KEY_LEFT)
    asm.instr("b", "skip_left", cond="NE")
    asm.instr("cmp_imm", 4, 0)
    asm.instr("sub_imm", 4, 4, MOVE_STEP, cond="NE")
    asm.label("skip_left")
    asm.instr("tst_imm", 2, KEY_UP)
    asm.instr("b", "skip_up", cond="NE")
    asm.instr("cmp_imm", 5, 0)
    asm.instr("sub_imm", 5, 5, MOVE_STEP, cond="NE")
    asm.label("skip_up")
    asm.instr("tst_imm", 2, KEY_DOWN)
    asm.instr("b", "skip_down", cond="NE")
...
def build_rom() -> bytes:
    ...
    rom[0xA0:0xAC] = b"GOOSCUBEV3  "
    rom[0xAC:0xB0] = b"GV53"
    rom[0xB0:0xB2] = b"01"
    rom[0xB2] = 0x96
    rom[0xBC] = 0x00
...
def main() -> None:
    out_path = Path(__file__).with_name("goose_cube_visualizer_055ad7_v3.gba")
    fallback_path = out_path.with_name("goose_cube_visualizer_055ad7_v3.next.gba")
