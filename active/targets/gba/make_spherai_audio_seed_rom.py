...
SQUARE_SIZE = 20
MOVE_STEP = 4
DOT1_INIT_X = (SCREEN_WIDTH // 4) - (SQUARE_SIZE // 2)
DOT2_INIT_X = ((SCREEN_WIDTH * 3) // 4) - (SQUARE_SIZE // 2)
DOT1_INIT_Y = 32
DOT2_INIT_Y = 96
...
def build_program() -> bytes:
    ...
    asm.instr("ldr_lit", 1, "lit_bg_color")
    asm.instr("bl", "fill_screen")
    asm.instr("bl", "draw_pixel_sphere")
    asm.instr("mov_imm", 10, SQUARE_SIZE)
    asm.instr("mov_reg", 0, 4)
    asm.instr("mov_reg", 1, 5)
    asm.instr("ldr_lit", 2, "lit_dot1_color")
    asm.instr("bl", "draw_square")
    asm.instr("mov_reg", 0, 6)
    asm.instr("mov_reg", 1, 7)
    asm.instr("ldr_lit", 2, "lit_dot2_color")
    asm.instr("bl", "draw_square")

    asm.label("frame_loop")
    asm.instr("bl", "wait_vblank")
    asm.instr("ldr_lit", 12, "lit_reg_keyinput")
    asm.instr("ldrh", 2, 12, 0)

    asm.instr("tst_imm", 2, KEY_START)
    asm.instr("b", "skip_reset", cond="NE")
    asm.instr("mov_imm", 4, DOT1_INIT_X)
    asm.instr("mov_imm", 5, DOT1_INIT_Y)
    asm.instr("mov_imm", 6, DOT2_INIT_X)
    asm.instr("mov_imm", 7, DOT2_INIT_Y)
    asm.label("skip_reset")
    ...
    asm.instr("cmp_imm", 4, MAX_X)
    asm.instr("add_imm", 4, 4, MOVE_STEP, cond="LT")
    asm.label("move_dot1_skip_right")
    asm.instr("tst_imm", 2, KEY_LEFT)
    asm.instr("b", "move_dot1_skip_left", cond="NE")
    asm.instr("cmp_imm", 4, 0)
    asm.instr("sub_imm", 4, 4, MOVE_STEP, cond="NE")
    asm.label("move_dot1_skip_left")
    asm.instr("tst_imm", 2, KEY_UP)
    asm.instr("b", "move_dot1_skip_up", cond="NE")
    asm.instr("cmp_imm", 5, 0)
    asm.instr("sub_imm", 5, 5, MOVE_STEP, cond="NE")
    asm.label("move_dot1_skip_up")
    asm.instr("tst_imm", 2, KEY_DOWN)
    asm.instr("b", "move_dot1_skip_down", cond="NE")
...
def build_rom() -> bytes:
    ...
    rom[0xA0:0xAC] = b"SPHERAI GBA "
    rom[0xAC:0xB0] = b"SGP1"
    rom[0xB0:0xB2] = b"01"
    rom[0xB2] = 0x96
    rom[0xBC] = 0x00
...
def main() -> None:
    out_path = Path(__file__).with_name("spherai_audio_seed.gba")
    fallback_path = out_path.with_name("spherai_audio_seed.next.gba")
